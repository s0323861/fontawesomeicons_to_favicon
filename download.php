...
// 色反転時は白いアイコンを、選択色の背景上に描画する
$invertColors = isset($_POST["invert"]) && $_POST["invert"] === "1";
...
foreach($icons as $iKey=>$iParam)
{
  ...
  if ($invertColors) {
    $invertedImage = imagecreatetruecolor($outputSize, $outputSize);
    $invertedBackground = imagecolorallocate(
      $invertedImage,
      $array_colorcode['red'],
      $array_colorcode['green'],
      $array_colorcode['blue']
    );
    imagefill($invertedImage, 0, 0, $invertedBackground);
    imagealphablending($invertedImage, true);
    imagecopy($invertedImage, $im, 0, 0, 0, 0, $outputSize, $outputSize);
    imagedestroy($im);
    $im = $invertedImage;
  }
